      ...
      <ModalBody>
        ...
        <Row tag="form" onSubmit={handleSubmit} className="mt-3">
          ...
          <Col xs={12} md={6}>
            ...
            <Select options={dropDownItem} getOptionLabel={(options) => options["name"]}
...
export const TakePhotoModal = (props) => {
  ...
  const contextArr = [{ name: "Document Front",
  context: "document-front"},{ name: "Document Back",
    ...
    <Modal isOpen={props.open}>
      ...
      <ModalBody>
        ...
        ><span className="align-middle ms-50">Document Context</span>
            <Select options={contextArr} getOptionLabel={(options) => options["name"]}
             getOptionValue={(options) => options["context"]}
          ...
          <Button onClick={()=>setToggle((oldState) =>!oldState)}>Switch</Button>
          {toggle?<>
          <Webcam
              audio={false}
              ref={webcamRef}
            ...
            {imgSrc && <img width="100%" height="100%" src={imgSrc} />}
  ...
  const getImage = () => {
    ...
    actions.getMedia({ mediaId: image.id, signature: hash }).then((res) => {
      if (res.data) {
        ...
        console.log( `data:image/png;base64, ${Buffer.from(res.data,"base64url")}`)
        setImg(
          `data:image/png;base64, ${Buffer.from(res.data,"base64")}`
        );
      }
    });
  };
